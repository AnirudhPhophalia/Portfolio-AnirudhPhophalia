...
export const themes = [
  { id: 'arnav-dark', name: 'Anirudh Dark', emoji: '💙', dot: '#2f81f7' },
...
export type Overlay = 'palette' | 'settings' | 'resume' | 'copilot' | null
...
type Action =
  ...
  | { type: 'CLOSE_OTHERS'; id: FileId }
  | { type: 'CLOSE_TO_RIGHT'; id: FileId }
  | { type: 'CLOSE_ALL' }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'CLOSE_OTHERS':
      return { ...state, tabs: [action.id], activeTab: action.id }
    ...
    case 'CLOSE_TO_RIGHT': {
      ...
      return {
        ...state,
        tabs,
        activeTab: tabs.includes(state.activeTab as FileId) ? state.activeTab : action.id,
      }
    }
